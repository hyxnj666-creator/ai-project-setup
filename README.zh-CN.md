...
> 一个 Cursor Skill，为任意项目初始化 AI 协作文档基建。

[English](./README.md) | 中文版

一句话，为任意项目生成完整的 AI 协作文档基建——AGENTS.md、MEMORY.md、ROADMAP.md、ADR 目录、Cursor Rules、架构骨架、踩坑记录。
...
将 `SKILL.zh.md` 放入你的 Cursor Skills 目录（中文版）：

```bash
# 个人全局使用（推荐）
~/.cursor/skills/ai-project-setup/SKILL.md   # 复制 SKILL.zh.md 内容即可
...
├── ROADMAP.md                        ← Phase 规划 + Definition of Done
├── docs/
...
├── .github/
│   └── copilot-instructions.md       ← Copilot 配置（Copilot 用户专用）
└── eval/
    └── eval.py | eval.js             ← AI 输出评测（有 AI 功能时生成）
```
